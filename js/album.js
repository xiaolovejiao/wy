document.addEventListener('DOMContentLoaded', () => {
    ...
    // 封面上传函数
    async function uploadCoverToCloud(file) {
        try {
    ...
    // 照片上传函数
    async function uploadPhotoToCloud(file, albumId) {
        try {
            const formData = new FormData();
    ...
    // 修改创建相册函数，使用云存储
    // 在相应的位置找到创建相册的代码，并替换为以下代码
    document.getElementById('createAlbumBtn').addEventListener('click', async function() {
        ...
        try {
            ...
            if (coverFile) {
                // 上传封面到云存储
                const uploadResult = await uploadCoverToCloud(coverFile);
            ...
            showAlert('成功', '相册创建成功', 'success');
        } catch (error) {
            console.error('创建相册失败:', error);
            showAlert('错误', '创建相册失败: ' + error.message, 'error');
        }
    });

    // 修改添加照片函数，使用云存储
    // 找到添加照片的代码，并替换为以下代码
    document.getElementById('submitAddPhoto').addEventListener('click', async function() {
        ...
        try {
            ...
            if (!albumId) {
                throw new Error('相册ID不存在');
            }
            
            // 上传照片到云存储
            const uploadResult = await uploadPhotoToCloud(photoFile, albumId);
            ...
            showAlert('成功', '照片添加成功', 'success');
        } catch (error) {
            console.error('添加照片失败:', error);
            showAlert('错误', '添加照片失败: ' + error.message, 'error');
        }
    });

    // 修改删除照片函数，删除云存储中的文件
    // 找到删除照片的代码，并替换或添加以下代码
    async function deletePhoto(photoId) {
        try {
            ...
            const photoIndex = albums[albumIndex].photos.findIndex(photo => photo.id === photoId);
            ...
            // 如果有云存储key，删除云存储文件
            if (photo.key) {
                await deleteCloudFile(photo.key);
            }
            ...
            showAlert('成功', '照片删除成功', 'success');
        } catch (error) {
            console.error('删除照片失败:', error);
            showAlert('错误', '删除照片失败: ' + error.message, 'error');
        }
    }

    // 修改删除相册函数，删除云存储中的文件
    // 找到删除相册的代码，并替换或添加以下代码
    document.getElementById('confirmDelete').addEventListener('click', async function() {
        try {
            ...
            // 删除封面文件
            if (album.coverKey) {
                await deleteCloudFile(album.coverKey);
            }
            ...
            for (const photo of album.photos) {
                if (photo.key) {
                    try {
                        ...
                        console.error(`删除照片文件失败: ${photo.key}`, error);
                    }
                }
            }
            ...
            showAlert('错误', '删除相册失败: ' + error.message, 'error');
        }
    });
}); 